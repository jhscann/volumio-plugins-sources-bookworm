...
When the selected Bluetooth device is connected, **Choose where music plays** shows two separate controls: **Bluetooth device volume** for the speaker/headphones hardware level, and **Volumio software volume** for Volumio's global digital level. Set the required values and choose **Apply volume settings**. Effective loudness depends on both. The plugin displays but does not change the global mixer mode; change Hardware or Software mixer mode only in Volumio **Playback Options**.
