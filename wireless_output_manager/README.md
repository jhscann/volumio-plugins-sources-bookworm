...
- before a manual routing change, the plugin captures Volumio's volume and mute state, then temporarily mutes at 0%. The saved volume is restored while still muted and must remain stable before the original mute state is restored. Bluetooth absolute-volume initialization can briefly override the value, so restoration uses bounded retries. If Volumio has disabled volume control, no volume command is sent. If restoration cannot be verified, playback remains stopped at 0% and muted, and the plugin tells you to set a safe volume manually before pressing Play.
